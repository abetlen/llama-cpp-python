...
print("low_level_last_logits_selected", {31: float(last_logits[31]), 916: float(last_logits[916])})
# Top10 tokens
top_indices = np.argsort(last_logits)[-10:][::-1]
print("low_level_top10", list(map(int, top_indices)))

# Now high-level API path
llama = llama_cpp.Llama(model_path, n_ctx=32, n_batch=32, n_ubatch=32, logits_all=True, flash_attn=True)
...
constructed_prompt_tokens = ([] if (not add_bos_flag) else [llama.token_bos()]) + hi_tokens_no
...
if llama._logits_all:
    ...
    print("high_level_prompt_last_logits_selected", {31: float(prompt_last_logits[31]), 916: float(prompt_last_logits[916])})
